/**
 * Das Modul besteht aus der Klasse {@linkcode FahrzeugFileService}, damit
 * Binärdateien mit dem Treiber von _MongoDB_ in _GridFS_ abgespeichert und
 * ausgelesen werden können.
 * @packageDocumentation
 */

import { FahrzeugNotExists, FileNotFound, MultipleFiles } from './errors';
import { closeMongoDBClient, connectMongoDB, logger, save } from '../../shared';
import { FahrzeugModel } from '../entity';
...
/**
 * Mit der Klasse {@linkcode FahrzeugFileService} können Binärdateien mit dem
 * Treiber von _MongoDB_ in _GridFS_ abgespeichert und ausgelesen werden.
 */
export class FahrzeugFileService {
    private readonly service = new FahrzeugService();

    /**
     * Asynchrones Abspeichern einer Binärdatei.
     *
     * @param id ID des zugehörigen Fahrzeuges, die als Dateiname verwendet wird.
    ...
    async save(id: string, buffer: Buffer, contentType: string | undefined) {
        logger.debug(
            'FahrzeugFileService.save(): id=%s, contentType=%s',
            id,
            contentType,
        );

        // Gibt es ein Fahrzeug zur angegebenen ID?
        // eslint-disable-next-line line-comment-position, spaced-comment
        const fahrzeug = await this.service.findById(id); //NOSONAR
    ...
    async find(filename: string) {
        logger.debug('FahrzeugFileService.findFile(): filename=%s', filename);
    ...
    private async deleteFiles(filename: string, bucket: GridFSBucket) {
        ...
        logger.debug('FahrzeugFileService.deleteFiles(): ids=%o', ids);
        ids.forEach((fileId) =>
            bucket.delete(fileId, () =>
                logger.debug(
                    'FahrzeugFileService.deleteFiles(): geloeschte ID=%o',
                    fileId,
                ),
            ),
        );
    ...
    private async checkFilename(filename: string) {
        ...
        // eslint-disable-next-line line-comment-position, spaced-comment
        const fahrzeug = await FahrzeugModel.findById(filename); //NOSONAR
        // eslint-disable-next-line no-null/no-null
        if (fahrzeug === null) {
            const result = new FahrzeugNotExists(filename);
            logger.debug(
                'FahrzeugFileService.checkFilename(): FahrzeugNotExists=%o',
                result,
            );
            return result;
        }
    ...
    private async getContentType(filename: string, bucket: GridFSBucket) {
        ...
        switch (files.length) {
            case 0: {
                ...
                logger.debug(
                    'FahrzeugFileService.getContentType(): FileNotFound=%o',
                    error,
                );
                return error;
            }
            ...
            case 1: {
                ...
                logger.debug(
                    'FahrzeugFileService.getContentType(): contentType=%s',
                    contentType,
                );
                return contentType;
            }
            ...
            default: {
                ...
                logger.debug(
                    'FahrzeugFileService.getContentType(): MultipleFiles=%o',
                    error,
                );
