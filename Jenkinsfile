    ...
    stages {
        ...
        stage('Test, Codeanalyse, Security, Dok.') {
            ...
            post {
                ...
                success {
                    script {
                        if (fileExists("${env.WORKSPACE}/fahrzeug.zip")) {
                            sh 'rm fahrzeug.zip'
                        }
                    }
                    // https://www.jenkins.io/doc/pipeline/steps/pipeline-utility-steps/#zip-create-zip-file
                    zip zipFile: 'fahrzeug.zip', archive: false, dir: 'dist'
                    // jobs/fahrzeug/builds/.../archive/fahrzeug.zip
                    archiveArtifacts 'fahrzeug.zip'
                }
            }
        }
        ...
        stage('Docker Image bauen') {
            steps {
              ...
              // sh 'docker build --tag juergenzimmermann/fahrzeug:1.0.0 .'
            }
        }
